...
class DataDriver:
    def __init__(self):
        ...
        self.face_crop = face_crop.FaceCrop()
        self.database = IDDatabase(self.predictor, self.face_crop)
        self.threshold = 95
    ...
    def lookup_person(self, comp_image):
        ...
        comp_image = self.face_crop.crop(comp_image)
        comp_image_vec = np.array(self.predictor.get_vec(comp_image)[0].tolist())
        min_dis = float("inf")
        matched_id = None
    ...
    def verify_id(self, id, comp_image, debug=False):
        ...
        dis = self.predictor.get_distance(
                comp_image_vec, np.array(id_vec)
        )
        if debug:
            print(dis)
